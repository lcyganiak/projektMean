...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    if (delta) {
      ...
      this.collection.updateOne(where, delta[1], saveOptions, function(
        err,
        ret
      ) {
        if (err) {
...
Model.find = function find(conditions, projection, options, callback) {
  ...
  if (
    connectionConfig &&
    typeof connectionConfig.useFindAndModify !== 'undefined'
  ) {
...
Model.findOneAndUpdate = function(conditions, update, options, callback) {
  ...
  const retainKeyOrder =
    get(options, 'retainKeyOrder') ||
...
function assignVals(o) {
  ...
  const retainNullValues = get(
    o,
    'allOptions.options.options.retainNullValues',
    false
  );
...
function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    while (k--) {
      ...
      try {
        Model =
          originalModel && originalModel.modelName
            ? originalModel
...
function valueFilter(val, assignmentOpts, populateOptions) {
  if (Array.isArray(val)) {
    ...
    for (let i = 0; i < numValues; ++i) {
      ...
      if (
        !isDoc(subdoc) &&
        (!populateOptions.retainNullValues || subdoc != null)
      ) {
        continue;
      }
...
Model.compile = function compile(
  ...
  model.$__insertMany = model.hooks.createWrapper(
    'insertMany',
    model.$__insertMany,
    model,
    kareemOptions
